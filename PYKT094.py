class NV:
    def __init__(self, ma, ten, luong, songay):
        ...
        if a == "A":
            if nam <= 3: self.hs = 10
            elif nam <= 8: self.hs = 12
            elif nam <= 15: self.hs = 14
            else: self.hs = 20
        if a == "B":
            if nam <= 3: self.hs = 10
            elif nam <= 8: self.hs = 11
            elif nam <= 15: self.hs = 13
            else: self.hs = 16
        if a == "C":
            if nam <= 3: self.hs = 9
            elif nam <= 8: self.hs = 10
            elif nam <= 15: self.hs = 12
            else: self.hs = 14
        if a == "D":
            if nam <= 3: self.hs = 8
            elif nam <= 8: self.hs = 9
            elif nam <= 15: self.hs = 11
            else: self.hs = 13
